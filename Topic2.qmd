...
![Data from the `astsa` package (Shumway & Stoffer).](sample_globaltemp.png)
...
![Data from the `astsa` package (Shumway & Stoffer).](sample_speech.png)

![Data from the `astsa` package (Shumway & Stoffer).](sample_disaster.png)

![Each panel overlays the BOLD traces at several locations; the step function is the periodic experimental stimulus. Data from the `astsa` package (Shumway & Stoffer).](sample_fmri.png)
...
![Both panels share a vertical scale, so the variance reduction from smoothing is visible directly. Simulated, following Shumway & Stoffer.](sample_wnma.png)
