  ...
  fluidRow(
    column(12,
      div(class = "ctrl-panel",
        ...
        div(style = "width: 100%;",
          sliderInput("theta",
                      label = NULL, min = -2, max = 2, value = 0.5,
...
- Here, we see for the two-variable case where the familiar variance 'rule' comes from. 
...
::: slide-footer
$\frac{1}{N}\sum_{i=1}^N Y_i$, the sample mean, is the most widely-used statistic in econometrics.
:::
:::::
...
::: slide-body
**Chebyshev's LLN.** Suppose $Y_t \overset{iid}{\sim} F_Y(\mu_Y, \sigma_Y^2)$ for $t = 1,\ldots,T$ where $0 < \sigma_Y^2 < \infty$. Then, it holds that $$\operatorname*{plim}_{T\to\infty}\bar{Y}_T = \mu_Y.$$

*Proof.* [See next slide.] 

- The generic "$F_Y$" notation denotes deliberate agnosticism about the distribution.

- The result is one of convergence in probability of $\bar{Y}_T$ to $\mu_Y$. What is convergence in probability? Estimator $\hat{\theta}_T$ is said to converge in probability to parameter $\theta$ if and only if there exists an arbitrarily small $e > 0$ such that $$\lim_{T\to\infty} \Pr\!\left(\left|\hat{\theta}_T - \theta\right| > e\right) = 0.$$ We alternatively say that $\hat{\theta}_T \overset{p}{\to} \theta$ as $T \to \infty$, or that $\hat{\theta}_T$ is **consistent** for $\theta$.
:::

::: slide-footer
A sample moment is a consistent estimator for its population counterpart — this is why the sample mean is so widely used. 
:::
:::::
